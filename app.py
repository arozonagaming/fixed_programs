...
# Secret key for session management not exposed
...
# hash and check passwords securely
...
# Iterate through the JSON data and hash the passwords
for key, value in data.items():
    hashed_password = hash_password(value)
    append_dict = {key: hashed_password}
    DEFAULT_CREDENTIALS.update(append_dict) 

# allow only certain file types to be uploaded: mitigates file upload vulnerability
...
def init_db():
    ...
    for username, password in DEFAULT_CREDENTIALS.items():
        ...
        if not c.fetchone():
            ...
            # credentials already hashed and added to database
...
def login():
    if request.method == "POST":
        ...
        # secret key is hashed and stored in static/secret_key.txt
        ...
        # session id is stored in a cookie, when logging out destrys session
        # cookie
...
def register_page():
    ...
    c = conn.cursor()
    # fixed SQL injection vulnerability
    # already escaped username and password
...
def admin():
    ...
    if request.method == "POST":
        # fixed XSS vulnerability thorugh CSP and CSP headers
...
def get_user(username):
    # limited to only admin to access user data: IDOR minimised
    if "user" not in session or session["user"] != "admin":
        return "Access Denied! <a href='/'>Go back</a>"
    try:
        conn = sqlite3.connect("users.db")
...
def debug_file(file_path):
    #  limited to only admin to access debug file: IDOR minimised
    if "user" not in session or session["user"] != "admin":
        return "Access Denied! <a href='/'>Go back</a>"
    try:
        import platform
